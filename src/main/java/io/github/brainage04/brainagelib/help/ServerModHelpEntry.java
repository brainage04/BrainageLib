...
public record ServerModHelpEntry(
        String modId,
        String displayName,
        String description,
        String helpCommand,
        String adminConfigCommand
) {
    ...
    public ServerModHelpEntry {
        ...
        helpCommand = requireCommand(helpCommand, "helpCommand");
        if (!MOD_ID_PATTERN.matcher(modId).matches()) {
            throw new IllegalArgumentException("Invalid mod id: " + modId);
        }
        if (adminConfigCommand != null) {
            adminConfigCommand = requireCommand(adminConfigCommand, "adminConfigCommand");
        }
    }

    public static ServerModHelpEntry playerOnly(
            String modId,
            String displayName,
            String description,
            String helpCommand
    ) {
        return new ServerModHelpEntry(modId, displayName, description, helpCommand, null);
    }

    private static String requireCommand(String value, String name) {
        String command = requireText(value, name);
