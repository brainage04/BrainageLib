...
public final class BrainageLibClientGameTest implements FabricClientGameTest {
    private static final ServerModHelpEntry PLAYER_HELP = ServerModHelpEntry.playerOnly("brainagelib-recording-player", "Recording Player Tools", "Deterministic player commands for this server.", "/recordingplayer help");
    ...
    private static void assertClientCommandTree(ClientGameTestContext context) { context.computeOnClient(client -> { if (client.getConnection() == null) throw new AssertionError("Expected a connected client for /servermods command verification."); var serverMods = client.getConnection().getCommands().getRoot().getChild(ServerModsCommand.COMMAND_NAME); if (serverMods == null || serverMods.getChild("help") == null || serverMods.getChild("config") == null) throw new AssertionError("Expected /servermods with help and config roots to be synchronized to the client."); return null; }); }
    private static void runCommand(ClientGameTestContext context, String command) { context.runOnClient(client -> { if (client.getConnection() == null) throw new AssertionError("Expected a connected client to run /" + command + '.'); client.getConnection().sendCommand(command); }); }
}
